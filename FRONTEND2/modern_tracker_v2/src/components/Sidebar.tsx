...
import {
  ...
  faChartPie // ADDED for Finance Icon
    ...
    { section: "ADMIN", items: [] },
    {
      items: [
        ...
        },
        {
          ...
          icon: faChartPie,
          requires_finance: true, // Custom flag to hide if no permission
          subItems: [
            { name: "General Ledger", path: "/finance#ledger" },
            { name: "AP & AR Aging", path: "/finance#apar" },
            { name: "Expense OCR", path: "/finance#expenses" },
          ...
          {menuItems.map((group, idx) => (
            <div key={idx}>
              ...
              {group.items &&
                group.items
                  .filter((item: any) => {
                    // Check module permissions against the serial key
                    if (item.requires_hr) {
                      return user?.serial_key?.allow_hr === true;
                    }
                    if (item.requires_finance) {
                      return user?.serial_key?.allow_finance === true;
                    }
                    return true;
                  })
                  .map((item: any) => (
